...
def income(update: Update, context: CallbackContext) -> None:
    try:
        ...
        update.message.reply_text('輸入的參數不足，請查看"/help income"')
    except ValueError:
        logger.info("User %s cause a ValueError", update.message.from_user.id)
        update.message.reply_text('格式輸入錯誤，請查看"/help income"')
    except Exception:
        logger.exception(f"unexpected error cause by user {update.message.from_user.id}")
        update.message.reply_text("未預期的錯誤")
    else:
        logger.info("User %s execute /income successfully", update.message.from_user.id)

def get_handler() -> CommandHandler:
    return CommandHandler('income', income)